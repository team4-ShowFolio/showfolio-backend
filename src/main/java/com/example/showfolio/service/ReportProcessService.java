...
public class ReportProcessService implements ReportProcessor {
    ...
    @Override
    @Transactional
    public void process(Long reportId, Long adminId, ProcessStatus status, String reason) {
        ...
        ReportProcessHandler handler = handlers.get(status);
        if (handler == null) {
            throw new IllegalArgumentException("지원하지 않는 처리 상태입니다: " + status);
        }

        handler.handle(report, adminId, reason);
    }
}
